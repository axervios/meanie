...
client.on("ready", () => {
  ...
  console.log("LP Helper is now online!");
...
client.on("guildCreate", guild => {
  ...
  channel.send(`LP Helper has now arrived in **${guild.name}**! It is a pleasure to make your acquaintance. :robot:`);
});
...
client.on("message", async message => {
    ...
    function log(cmd) {
      ...
      let logchannel = client.channels.find(c => c.id === "436702599706705940");
    ...
    if (command === "commands" || command === "help" || mentionMessage && mentionMessage.includes("commands") || mentionMessage && mentionMessage.includes("help")) {
      ...
      message.author.send({ embed: {
        ...
        fields: [
          ...
          {
            name: "---------- LP Site/SMF Commands ----------",
            value: "These commands are used to automate various actions on the Level Palace site, or with Super Mario Flash."
          },
          // {
    ...
    // }
      ...
      let channel = client.channels.find(c => c.id === "325490421419606016");
    ...
    //   } else {
    //     let starMsg = await message.channel.fetchMessages({ limit: 100 }).catch(err => {
    ...
    // }
      ...
      message.channel.send("Test!").then(msg => {
        ...
        msg.edit(`I am currently working!\n\`Latency:\` ${msg.createdTimestamp - message.createdTimestamp}ms.\n\`API Latency:\` ${Math.round(client.ping)}ms.`);
      });
    }
    ...
    // }
            ...
            $("div.card-content", "div.table-container").each(function() {
              if ($(this).find("a.card-title").length > 0) {
                if (levels.length <= 9) {
                  let levellink = $(this).find("a[href*='level']").attr("href");
                  levellinks.push(levellink);
                  let level = $(this).find("a[href*='level']").text().trim();
                  levels.push(level);
                }
              }
            });
      ...
      if (args.trim().toLowerCase() === "remove") {
        ...
        message.member.removeRole(colorRole.id).then(role => {
        ...
        if (!message.guild.roles.find(r => r.name === color)) {
          let oldColorRole = message.member.colorRole;
          if (oldColorRole) {
            message.member.removeRole(oldColorRole.id).then(user => {
          ...
          message.guild.createRole({
            ...
            message.member.addRole(newColorRole).catch(err => {
          ...
          let newColorRole = message.guild.roles.find(r => r.name === color);
          
          if (message.member.roles.has(newColorRole.id)) return message.channel.send("You already have that color role!");
          
          let oldColorRole = message.member.colorRole;
          if (oldColorRole) message.member.removeRole(oldColorRole.id).catch(err => {
            console.error(`An error occurred while trying to remove a role from a user in #${message.channel.name} of ${message.guild.name}:\n${err}`);
            return message.channel.send("**An error occurred while trying to remove a role from a user:**```" + err + "```");
          });
      ...
      urlExists(emojiURL, function(err, exists) {
        if (!err) {
          if (exists) {
            if (message.guild.emojis.size === 50) return pmsg.edit("There are already **50** emojis in this server, and no more can be added!");
              message.guild.createEmoji(emojiURL, emojiName).then(emoji => {
      ...
      if (isNaN(simplifiedArgs)) {
        emoji = message.guild.emojis.find(e => e.name.toLowerCase() === simplifiedArgs);
      } else {
        emoji = message.guild.emojis.find(e => e.id === simplifiedArgs.replace(/\s/g, ""));
      }
      ...
      if (!emoji) {
        ...
        message.guild.deleteEmoji(emoji).catch(err => {
      ...
      let slotsleft = 50 - message.guild.emojis.size;
      if (slotsleft < 0) slotsleft = 0;
      if (message.guild.id === "325490421419606016") slotsleft = 53 - message.guild.emojis.size;
      ...
      let slotsleft = 250 - message.guild.roles.size;
      ...
      if (args.replace(/\s/g, "").toLowerCase() === "random") {
        ...
        request({ uri: `http://levelpalace.wikia.com/wiki/${articleNameEncoded}`, timeout: 10000, headers: { "User-Agent": "lp-helper" } }, function(err, resp, body) {
          ...
          if (!err && resp.statusCode === 200) {
            ...
            if (articleinfo[1]) {
              if (message.channel.type === "dm") {
                console.log(`${message.author.tag} requested a random LP wiki article by the name of "${articleName}" in DMs, which does exist.`);
              } else if (message.channel.type === "group") {
                console.log(`${message.author.tag} requested a random LP wiki article by the name of "${articleName}" in the ${message.channel.name} Group DM, which does exist.`);
              } else {
                console.log(`${message.author.tag} requested a random LP wiki article by the name of "${articleName}" in #${message.channel.name} of ${message.guild.name}, which does exist.`);
              }
              pmsg.edit({ embed: {
                color: 3447003,
                fields: [
          ...
          // do this if the article doesn't exist
            ...
            if ($(".alternative-suggestion").length > 0) {
              ...
              if (message.channel.type === "dm") {
                console.log(`${message.author.tag} requested a random LP wiki article by the name of "${articleName}" in DMs, which does not exist.`);
              } else if (message.channel.type === "group") {
                console.log(`${message.author.tag} requested a random LP wiki article by the name of "${articleName}" in the ${message.channel.name} Group DM, which does not exist.`);
              } else {
                console.log(`${message.author.tag} requested a random LP wiki article by the name of "${articleName}" in #${message.channel.name} of ${message.guild.name}, which does not exist.`);
              }
    ...
    //     console.log(`${message.author.tag} didn't correctly use an LP Helper command, by entering "${config.prefix + command}".`);
    //     message.channel.send("**" + config.prefix + command + "** is not an LP Helper command, use `" + config.prefix + "commands` or `" + config.prefix + "help` for a list of ones you can use.", { split: true }).then(msg => {
    //       msg.delete(10000);
    //     }).catch(err => {
    //       console.error(`An error occurred while trying to send a message:\n${err}`);
    //       return message.channel.send("**An error occurred while trying to send a message:**```" + err + "```");
...
client.on("userUpdate", (oldUser, newUser) => {
  ...
  if (oldUser.id === config.creatorid && oldUser.displayAvatarURL !== newUser.displayAvatarURL) {
    let tserver = client.guilds.find(g => g.id === "200458694704627712");
    tserver.setIcon(newUser.displayAvatarURL, "MB avatar change occurred.");
  }
});
...
client.on("messageReactionAdd", async (reaction, user) => {
  ...
  if (reaction.emoji.name === '⭐') {
    ...
    if (message.channel.type === "dm" || message.channel.type === "group") return;
    if (message.guild.id !== "325490421419606016") return;
    if (message.channel.id === "326576626097979402" || message.channel.id === "325490421419606016" || message.channel.id === "325496108467879956" || message.channel.id === "357386246793723905" || message.channel.id === "414852683292213253" || message.channel.id === "468472403199000576" || message.channel.id === "456626411910856706" || message.channel.id === "456626483654557698" || message.channel.id === "455993305067683840" || message.channel.id === "454493943426449408" || message.channel.id === "399778380398198784" || message.channel.id === "392421139089063937" || message.channel.id === "325495534896807936" || message.channel.id === "325495670502850560" || message.channel.id === "334144051706331136" || message.channel.id === "330810124757368832") return;
